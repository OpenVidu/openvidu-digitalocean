...
# SCALE IN: average CPU too low and above minimum — graceful infinite drain
  ...
  if [ -n "$LOWEST_ID" ]; then
    ...
    # Remove from active pool — Master Node stops sending new sessions here
...
CONFIG_S3_EOF
  ...
  # Store SSH key locally for the autoscaler (all master nodes receive it so the autoscaler can SSH to media nodes regardless of which master node runs it)
  mkdir -p /root/.ssh
  cp /tmp/openvidu_ssh_key_elastic.pem /root/.ssh/id_rsa
  ...
  # restart.sh on reboot + autoscaler cron (dynamic mode only)
