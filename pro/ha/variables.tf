...
variable "volumeSize" {
  description = "Size of the additional volume in GB for Master Node"
  type        = number
  default     = 100
}
...
variable "spacesAccessId" {
  description = "Access key ID for DigitalOcean Spaces (S3-compatible). Required if spaceAppDataName or spaceClusterDataName is empty (a new bucket will be created)."
  type        = string
  default     = ""
}

variable "spacesSecretKey" {
  description = "Secret access key for DigitalOcean Spaces (S3-compatible). Required if spaceAppDataName or spaceClusterDataName is empty (a new bucket will be created)."
  type        = string
  default     = ""
}
