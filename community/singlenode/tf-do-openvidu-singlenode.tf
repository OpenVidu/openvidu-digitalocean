...
CONFIG_S3_EOF
  ...
  echo "${tls_private_key.openvidu_ssh_key.private_key_openssh}" > /tmp/openvidu_ssh_key_sn.pem
  chmod 600 /tmp/openvidu_ssh_key_sn.pem
  
  # Upload private key to the bucket
  aws s3 cp /tmp/openvidu_ssh_key_sn.pem \
  s3://${var.spaceName == "" ? digitalocean_spaces_bucket.openvidu_space[0].name : var.spaceName}/openvidu_ssh_key_sn.pem \
  --endpoint-url=https://${var.spaceRegion}.digitaloceanspaces.com \
  --acl private \
  --region=${var.spaceRegion}
  
  # Clean up
  rm -f /tmp/openvidu_ssh_key_sn.pem
